...
$botones_herramientas["detalleFactura"]=true;
...
<!-- /Scripts extra -->
	...
	<script>
		...
		function cargarTabla(){
			$.ajax({
				url:'reportes/detalleFactura.php',
				data:'action=rep_facs', dataType:'json', type:'POST',
		...
		var manto = {
			...
			detalle:function(id){
				this.estado = 'editar';
				$('#modalHead').html("Detalle Factura");
				this.id = id;
				$.ajax({
					url:'reportes/detalleFactura.php',
					data:'action=rep_det&id='+id, dataType:'json', type:'POST',
					complete:function(datos){
						$('#AgregarDepto').modal('show');
						$(".content_fac").html(datos.responseText);
					}
				});
