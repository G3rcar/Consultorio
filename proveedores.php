...
<!-- /Scripts extra -->
	...
	<script>
		...
		$(document).ready(function(){
			...
			$('#guardarProv').click(function(){ manto.guardar(); });
			cargarLista();
	 ...
	 function validarForm(){
			...
			var iv5 = $('#telefonoProv').val(); 
			var iv6 = $('#calleProv').val(); 
			var iv7 = $('#coloniaProv').val(); 
			
			if(iv1==''){ $('#s2id_idPais').addClass('error_requerido_sel2'); errores++; }
			if(iv2==''){ $('#s2id_idDepto').addClass('error_requerido_sel2'); errores++; }
			if(iv3==''){ $('#s2id_idMuni').addClass('error_requerido_sel2'); errores++; }
			if(iv4==''){ $('#nombreProv').addClass('error_requerido'); errores++; }
			if(iv4.length>45){ $('#nombreProv').addClass('error_requerido').attr('title','No debe sobrepasar los 45 caracteres'); errores++; }
			if(iv5==''){ $('#telefonoProv').addClass('error_requerido'); errores++; }
			if(iv6==''){ $('#calleProv').addClass('error_requerido'); errores++; }
			if(iv7==''){ $('#coloniaProv').addClass('error_requerido'); errores++; }
			if(errores>0){
		...
		function limpiarValidacion(conTexto){
			...
			$('#s2id_idMuni').removeClass('error_requerido_sel2');
...
//Esta funcion me sirve para editar los datos de cada fila
			...
			guardar:function(){
				...
				var datos = 'action=sv_prov&nombre='+nombre+'&idMuni='+idMuni+ '&correo='+correo+ '&condominio='+condominio+'&calle='+calle+
				                    '&casa='+casa+'&colonia='+colonia+'&referencia='+referencia+'&tel='+tel+'&fax='+fax+'&id='+this.id;
